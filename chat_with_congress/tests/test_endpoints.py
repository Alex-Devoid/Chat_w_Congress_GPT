...
def test_search_members():
    with patch("chat_with_congress.app.api.services.congress_api.requests.get") as mock_get:
        ...
        response = client.post("/search-members/", params={"name": "John Doe"})
        ...
        response = client.post("/search-members/", params={"name": "Jane Doe"})
...
def test_get_member_details():
    with patch("chat_with_congress.app.api.services.congress_api.requests.get") as mock_get:
        ...
        response = client.post("/member-details/", params={"member_id": "A000360"})
        assert response.status_code == 200
...
def test_chat():
    with patch("chat_with_congress.app.api.services.congress_api.requests.get") as mock_get:
        ...
        response = client.post("/chat/", params={"question": "Tell me about the roles", "member_id": "A000360"})
        assert response.status_code == 200
